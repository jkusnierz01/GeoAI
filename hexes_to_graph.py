...
Input: a GeoJSON file created by your previous script (one resolution),
       containing features with properties like: h3_id, resolution, <amenity keys...>, total_count

Output: a PyTorch Geometric Data object saved to disk (graph_res{res}.pt)

Usage example:
    python3 hexes_to_graph.py --input amenities_hexagons_res8.geojson --out graph_res8.pt
"""
...
def extract_amenity_columns(gdf: gpd.GeoDataFrame) -> list:
    """Return all numeric columns except reserved ones."""
    reserved = {'h3_id','geometry','resolution','total_count'}
    return sorted([c for c in gdf.columns if c not in reserved and gdf[c].dtype.kind in 'fi'])

def compute_centroids(gdf: gpd.GeoDataFrame) -> np.ndarray:
    """Compute centroid coordinates (longitude, latitude) without normalization."""
    # optional: project to EPSG:3857 to avoid geographic warning
...
def build_h3_adjacency(h3_ids: list[str]) -> np.ndarray:
    ...
    edge_index = np.vstack([edge_list.T, edge_list.T[::-1]])
    return edge_index
...
def build_graph_from_geojson(path_geojson: str, include_coords: bool = False) -> Data:
    ...
    # Node features = just amenity counts
    ...
    # Store extra info for mapping
    props = [{k: row[k] for k in gdf.columns if k not in ['geometry']} for _, row in gdf.iterrows()]
...
def parse_args():
    p = argparse.ArgumentParser(description="Convert H3 hex GeoJSON into PyG graph for GraphMAE2.")
    p.add_argument("--input", "-i", required=True, help="Input GeoJSON file (one resolution)")
    p.add_argument("--out", "-o", required=True, help="Output file (e.g. graph_res8.pt)")
    p.add_argument("--no-normalize", dest="normalize", action="store_false", help="Disable feature normalization")
    p.add_argument("--no-coords", dest="coords", action="store_false", help="Do not include centroid coordinates in features")
    return p.parse_args()
